...
void *client_thread(void *param)
{
    ...
    if (strncmp(buff, "GET /get", 8) == 0)
    {
        ...
        if (strncmp(query, "file=", 5) == 0)
        {
            ...
            if ((strcmp(extension, ".txt") == 0) || (strcmp(extension, ".c") == 0) || (strcmp(extension, ".cpp") == 0))
            {
                ...
                if (f == NULL)
                {   char *error = "<html><h1>Error open File</h1></html>";
        ...
        else if (strncmp(query, "dir=", 4) == 0)
        {
            ...
            char *message = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\n\r\n<html>";
            send(clientSocket, message, strlen(message), 0);
            sprintf(buff, "<h1><a style=\"font-weight:900;color:#FF0000\" href=\"http://localhost:%d/get?dir=%s\">.</a></h1>", port, directory);
            send(clientSocket, buff, strlen(buff), 0);
            char *tmp;
            if ((tmp = getParentPath(directory)) != NULL)
            {
                sprintf(buff, "<h1><a style=\"font-weight:900;color:#FF0000\" href=\"http://localhost:%d/get?dir=%s\">..</a></h1>", port, tmp);
                send(clientSocket, buff, strlen(buff), 0);
            }
            ...
            while ((entry = readdir(dir)) != NULL)
            {
                if (strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0)
                {
                    ...
                    {
                        sprintf(buff, "<h1><a style=\"font-weight:900;color:#FF0000\" href=\"http://localhost:%d/get?dir=%s/%s\">%s</a></h1>", port, directory, entry->d_name, entry->d_name);
                        send(clientSocket, buff, strlen(buff), 0);
                    }
                }
            }
    ...
    else
    {
        ...
        char *message = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\n\r\n<html>";
        send(clientSocket, message, strlen(message), 0);
        sprintf(buff, "<h1><a style=\"font-weight:900;color:#FF0000\" href=\"http://localhost:%d\">.</a></h1>", port);
        send(clientSocket, buff, strlen(buff), 0);
        char *tmp;
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            if (strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0)
            {
                ...
                {
                    sprintf(buff, "<h1><a style=\"font-weight:900;color:#FF0000\" href=\"http://localhost:%d/get?dir=%s/%s\">%s</a></h1>", port, cwd, entry->d_name, entry->d_name);
                    send(clientSocket, buff, strlen(buff), 0);
                }
            }
        }
